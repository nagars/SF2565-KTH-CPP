...
#define NUM_THREADS		4		// Number of threads in parallel implementation

#define THREADS_ENABLE	1
#define THREADS_DISABLE	0
...
// Cumulative probability of extinction - P(t>s)
// Calculate P(t>s), according to equation (2) in assignment
std::vector<double> extinctionTimesParallel(std::vector<double>& times, double b) {
	...
	for (int i = 0; i < NUM_THREADS; ++i) {
		...
		taskflow.emplace(
			...
			// For each IC (m) in ICsamples (M)
			for (int m = i*M/NUM_THREADS; m < M/NUM_THREADS*(i+1); m++) {
...
std::vector<double> probabilityExtinctionTimes(std::vector<double>& times,
		double b, bool enable_threads) {

	// Generates vector of extinction times
	std::vector<double> r_extinctionTimes;
	if(enable_threads){
		r_extinctionTimes = extinctionTimesParallel(times, b);
	}else{
